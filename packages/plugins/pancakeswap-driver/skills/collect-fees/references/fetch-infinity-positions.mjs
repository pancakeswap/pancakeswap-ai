...
function feeGrowthInside(
	...
	const fgBelow =
		currentTick >= tickLower ? fgOutLower : (fgGlobal - fgOutLower + MOD) % MOD;
	const fgAbove =
		currentTick < tickUpper ? fgOutUpper : (fgGlobal - fgOutUpper + MOD) % MOD;
...
if (activeCLRows.length > 0) {
	...
	for (let i = 0; i < uniquePoolIds.length; i++) {
		...
		if (fgResult.status !== "success" || slot0Result.status !== "success")
			continue;
	...
	for (let i = 0; i < positionData.length; i++) {
		...
		if (
			tickLowerResult.status !== "success" ||
			tickUpperResult.status !== "success"
		)
			continue;

		console.log("tick results", tickLowerResult, tickUpperResult);

		const { feeGrowthOutside0X128: fgOutLower0, feeGrowthOutside1X128: fgOutLower1} = tickLowerResult.result;
		const { feeGrowthOutside0X128: fgOutUpper0, feeGrowthOutside1X128: fgOutUpper1} = tickUpperResult.result;
		...
		const pending0 =
			(((fg0Inside - fg0InsideLast + MOD) % MOD) * liquidity) / Q128;
		const pending1 =
			(((fg1Inside - fg1InsideLast + MOD) % MOD) * liquidity) / Q128;
...
const binRows = await fetchAllPages(
	`${EXPLORER}/infinityBin/${chainName}/poolsByOwner/${WALLET}`,
);
...
const poolMetaResults = await Promise.all(
	uniqueBinPoolIds.map((poolId) =>
		fetch(
			`https://explorer.pancakeswap.com/api/cached/pools/infinityBin/${chainName}/${poolId}`,
		)
...
for (const row of binRows) {
	...
	for (const bin of row.reserveOfBins ?? []) {
		...
		const amountX =
			totalShares > 0n ? (userShares * reserveX) / totalShares : 0n;
		const amountY =
			totalShares > 0n ? (userShares * reserveY) / totalShares : 0n;
