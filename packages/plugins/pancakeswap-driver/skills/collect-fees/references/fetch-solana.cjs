...
const PANCAKE_CLMM_PROGRAM_ID = new PublicKey(
...
async function fetchPoolInfos(poolIds) {
  ...
  const response = await fetch(
    `${apiBaseUrl}${searchUrl}?ids=${poolIds.join(",")}`,
  );
  ...
  return poolInfo.data.map((pool) => {
    ...
    if (pool.rewardDefaultInfos && pool.rewardDefaultInfos.length > 0) {
      isFarming = pool.rewardDefaultInfos.some(
        (reward) =>
          Number(reward.endTime ?? 0) * 1000 > timestamp &&
          reward.perSecond > 0,
      );
    }
...
async function getRewardInfo(connection, raydium, position) {
  ...
  const tickLowerState =
    ...
    TickUtils.getTickOffsetInArray(
      position.tickLower,
      result.computePoolInfo.tickSpacing,
    )
    ];
...
async function main() {
  ...
  const poolInfos = await fetchPoolInfos(
    parsedInfo.map((i) => i.poolId.toBase58()),
  );
